...
pub fn expand(allocator: Allocator, str: []const u8, getenv: *const @TypeOf(std.posix.getenv)) ![]const u8 {
    ...
    for (matches.items) |match| {
        ...
        if (cursor < start) {
            try res.appendSlice(allocator, str[cursor..match.getStartAt(0).?]);
        }
        // without dollar sign
        const val = getenv(match_text[1..]) orelse "";
        try res.appendSlice(allocator, val);
        cursor = end;
    }
    // append rest of the string
    try res.appendSlice(allocator, str[cursor..]);
